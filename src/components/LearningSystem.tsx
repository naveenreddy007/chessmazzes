  ...
  return (
    <section className="relative py-10 sm:py-16 lg:py-24">
      <div className="max-w-[1440px] mx-auto px-6 sm:px-12 lg:px-16" ref={ref}>
        ...
        >
          <div className="h-1 bg-white/5 rounded-full mx-auto max-w-2xl">
            <motion.div
              initial={{ width: "0%" }}
          ...
          <div className="flex justify-between max-w-2xl mx-auto mt-3">
            {levels.map((l) => (
              <span key={l.level} className="text-xs text-white/30 font-medium">{l.level}</span>
            ))}
          </div>
        </motion.div>
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {levels.map((level, i) => (
            <motion.div
              ...
              className="group"
            >
              <div className="glass-card p-7 h-full hover:border-white/15 transition-all duration-300 hover:-translate-y-1 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-white/30">{level.rating} ELO</span>
                  </div>
                </div>

                <ul className="space-y-3">
                  {level.modules.map((mod) => (
                    <li key={mod} className="flex items-center gap-2 text-sm text-white/50 group-hover:text-white/65 transition-colors">
